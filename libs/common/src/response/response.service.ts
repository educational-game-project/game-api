...
export class ResponseService {
    ...
    error(statusCode: number, message: string, errors?: RMessage): IResponseError {
        if (errors) {
            return {
    ...
    validationError(statusCode: number, errors: ValidationError[]): any {
        const response = {
            ...
            server_time: new Date().toISOString(),
            message: []
        }; for (const err of errors) Object.values(err.constraints).map(cons => { response.message.push(cons) });
        return response;
    }
}
